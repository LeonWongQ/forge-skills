...
|------------|-------|---------|------------|
| **Forge-root-relative** | References inside `.claude/forge/` | `behaviors/review.md`, `/runtime/router.md` | Relative to `.claude/forge/` |
...
In documentation inside `.claude/forge/`:
- `/` prefix (e.g., `/behaviors/review.md`) means forge-root-relative
- Bare paths (e.g., `behaviors/review.md`) also mean forge-root-relative

In documentation outside `.claude/forge/` (e.g., project-root CLAUDE.md, skills/ SKILL.md files):
- Full `.claude/forge/` or `.claude/skills/` prefixes are required for cross-boundary references
