...
def resolve_root(root: Optional[str]) -> Path:
    ...
    raise click.UsageError(
        ...
        "Run from a project with .codex/forge, .claude/forge, or .cursor/forge, "
...
def workspace_prefix_from_forge_root(root: Path) -> Optional[str]:
    ...
    Forge source lives below `.claude`, while an installed Codex copy lives
    below `.codex`. An unrecognized root has no host-specific prefix and must
    use a relative path rather than silently claiming to be a Claude install.
    """
