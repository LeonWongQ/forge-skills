...
1. **How to detect your model**: Look for model identity in your system prompt. In Claude Code, check for text like "You are powered by the model deepseek-v4-pro" or check the `<env>` block for `model` / `cliModel` fields. If running on a third-party platform (OpenRouter, DeepInfra, SiliconFlow, etc.), the model name is typically in the first system message.
...
|-------|:------:|:------------:|
| Claude Haiku 4.x | 200K | ~160K |
| Claude Sonnet 4.x | 200K (1M beta) | ~160K |
| Claude Opus 4.x | 200K (1M beta) | ~160K |
| Claude 3.5 Sonnet / Haiku | 200K | ~160K |
...
- Save critical findings to `.claude/memory/` or write to files
