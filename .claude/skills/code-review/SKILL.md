...
3. For every other review, load `.claude/forge/CLAUDE.md`, `.claude/forge/AUTOLOAD.md`, and classify with `.claude/forge/runtime/review-routing/review-routing-spec.md`.
4. Detect technical domains for the full path:
   - Java imports/annotations → `.claude/forge/domains/java.md`
   - Spring annotations (`@Service`, `@Transactional`, `@Autowired`) → `.claude/forge/domains/spring.md`
   - SQL / JDBC / MyBatis → `.claude/forge/domains/mysql.md`
   - Redis / cache → `.claude/forge/domains/redis.md`
...
| Module | Path | Role |
|--------|------|------|
| Behavior | `.claude/forge/behaviors/review.md` | Primary: evaluation, judgment, risk detection |
| Domains | Detected from artifact | Technical heuristics and failure modes |
| Template | `.claude/forge/templates/review-report.md` | Output structure: findings, severity, evidence, impact |
| Checklists | `.claude/forge/checklists/review-checklist.md` | Review quality: prioritized, evidence-based, actionable |
| | `.claude/forge/checklists/general-quality.md` | Baseline: facts vs assumptions, clarity, completeness |
| | `.claude/forge/checklists/verification-checklist.md` | Claims are justified and bounded |
| | `.claude/forge/checklists/delivery-checklist.md` | Result is usable before output |
...
See also: `.claude/skills/code-review/references/output-guard.md`
