...
3. Detect technical domains from the subject:
   - Java code/docs → `.claude/forge/domains/java.md`
   - Spring code/docs → `.claude/forge/domains/spring.md`
   - Database docs → `.claude/forge/domains/mysql.md`
   - Cache/Redis docs → `.claude/forge/domains/redis.md`
   - Test docs → `.claude/forge/domains/testing.md`
...
| Module | Path | Role |
|--------|------|------|
| Behavior | `.claude/forge/behaviors/document.md` | Primary: accuracy, clarity, audience fit |
| Domains | Detected from subject | Domain-specific terminology, conventions, context |
| Template | `.claude/forge/templates/default.md` | Flexible output shape (documentation is format-diverse) |
