...
3. Detect technical domains from the topic:
   - Spring concepts → `.claude/forge/domains/spring.md`
   - Java language/runtime → `.claude/forge/domains/java.md`
   - Database topics → `.claude/forge/domains/mysql.md`
   - Cache topics → `.claude/forge/domains/redis.md`
   - Testing topics → `.claude/forge/domains/testing.md`
...
| Module | Path | Role |
|--------|------|------|
| Behavior | `.claude/forge/behaviors/explain.md` | Primary: concept clarification, mental model building |
| Domains | Detected from topic | Domain-specific context and examples |
| Template | `.claude/forge/templates/explanation.md` | Output: what → why → how → example → pitfalls |
| Checklists | `.claude/forge/checklists/general-quality.md`, `.claude/forge/checklists/delivery-checklist.md` | |
