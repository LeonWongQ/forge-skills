...
> **Path convention**: Paths prefixed with `/` below are relative to the forge root (`.forge-skill/forge/`). For example, `/behaviors/review.md` resolves to `.forge-skill/forge/behaviors/review.md`. Skill references outside forge use the full `.claude/skills/` path.
...
```bash
.forge-skill/forge/scripts/run-local-checks.sh --quick
```
...
```bash
chmod +x .forge-skill/forge/scripts/run-local-checks.sh
.forge-skill/forge/scripts/run-local-checks.sh --full
```

**Windows CMD:**
```bat
.forge-skill\forge\scripts\run-local-checks.bat --full
```
...
| Reports not generated | `.forge-skill/forge/out/` writable? Python executable working? `python -m forge_cli --root . doctor --report ...` succeeds? |

### Recommended Workflow

```bash
# After any forge config or routing change:
.forge-skill/forge/scripts/run-local-checks.sh   # or .bat on Windows
