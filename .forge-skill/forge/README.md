...
**Windows CMD:**
```bat
.forge-skill\forge\scripts\run-local-checks.bat --quick
```

### Full Run

Run `.forge-skill/forge/scripts/run-local-checks.sh --full` or
`.forge-skill\forge\scripts\run-local-checks.bat --full` before release and in CI.
...
```bash
python -m forge_cli --root .forge-skill/forge --format json validate-output \
...
```bash
python -m forge_cli --root .forge-skill/forge validate
python .forge-skill/forge/scripts/check-route-regression.py
python -m pytest .forge-skill/forge/tests
```
