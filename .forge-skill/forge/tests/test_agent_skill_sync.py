...
@pytest.mark.skipif(os.name != "nt", reason="exercises the Windows batch wrapper")
def test_top_level_wrapper_preserves_installer_failure_exit_code():
    wrapper = Path(__file__).resolve().parents[3] / "install-skills.bat"

    completed = subprocess.run(
        ["cmd.exe", "/d", "/c", str(wrapper), "--client", "invalid", "--scope", "global"],
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
...
def test_top_level_wrapper_requires_supported_python_version():
    wrapper = Path(__file__).resolve().parents[3] / "install-skills.bat"
    content = wrapper.read_text(encoding="utf-8")

    assert "sys.version_info >= (3, 11)" in content
    assert "Python 3.11 or later is required" in content
