...
Load `.forge-skill/forge/domains/mysql.md` only for confirmed MySQL-compatible targets. Add Java, Spring, Redis, or testing domains only when the application evidence requires them. Read [references/data-design-guide.md](references/data-design-guide.md) for detailed schema, index, query, and migration guidance.
