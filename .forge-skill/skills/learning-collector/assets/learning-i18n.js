(() => {
  ...
  const messages = {
    "zh-CN": {
      ...
      hookConfiguration: "采集 Hook", hookConfigurationNote: "为本机用户选择一个宿主。各项目仍通过 Skill 开关独立决定是否采集，Skill 约束采集继续作为兜底。",
      codexTrustTitle: "Codex 用户 Hook 信任", codexTrustSteps: "应用 Codex Hook 后，打开 Codex Desktop → Settings → Hooks，找到用户 Hook，检查命令并点击 Trust / 信任，然后新建 Codex 会话。SessionStart 不会在已有会话中补执行。",
    ...
    en: {
      ...
      hookConfiguration: "Collection Hook", hookConfigurationNote: "Select one host for this local user. Each project still controls collection through its enabled Skills, and Skill-contract capture remains the fallback.",
      codexTrustTitle: "Trust the Codex user Hook", codexTrustSteps: "After applying the Codex Hook, open Codex Desktop → Settings → Hooks. Find the user Hook, review its command and select Trust, then start a new Codex session. SessionStart does not run retroactively in an existing session.",
