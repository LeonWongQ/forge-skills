(() => {
  ...
  const messages = {
    "zh-CN": {
      ...
      hookConfiguration: "采集 Hook", hookConfigurationNote: "一次只启用一个 Forge 采集 Hook。切换宿主会移除前一宿主的 Forge Hook，但保留用户的其他 Hook；各项目仍通过 Skill 开关独立决定是否采集。",
      codexTrustTitle: "Codex 用户 Hook 信任", codexTrustSteps: "应用 Codex Hook 后，打开 Codex Desktop → Settings → Hooks，找到用户 Hook，检查命令并点击 Trust / 信任，然后新建 Codex 会话以重新加载 Hook 配置。",
      ...
      claudeTrustTitle: "Claude Code Hook 状态", claudeTrustNote: "当前页面只验证配置文件和 Hook 回执；Claude Code 的原生信任或审批状态尚未验证，因此不会显示为“已信任”。",
      cursorTrustTitle: "Cursor Hook 状态", cursorTrustNote: "Cursor 原生 Hook 仍处于验证阶段；页面只报告配置和已观察到的回执，不把未知状态显示为失败。",
      globalSetting: "本机用户设置",
      codexHost: "Codex", claudeCodeHost: "Claude Code", cursorHost: "Cursor",
      selectedGlobally: "当前本机选择", globalHookReady: "本机用户 Hook 已安装",
      ...
      applyHook: "应用选择", removeHook: "移除本机 Hook",
      hookConfigured: "已将本机采集宿主切换为 {host}", hookRemoved: "已移除本机采集 Hook",
      ...
      removeHookConfirm: "移除本机用户采集 Hook？Skill 约束采集仍会保留。",
      switchHookConfirm: "切换到 {next} 会移除 {previous} 的 Forge Hook，但保留其他用户 Hook。继续吗？",
      hookSwitched: "已从 {previous} 切换到 {next}；前一宿主的 Forge Hook 已移除",
    ...
    en: {
      ...
      hookConfiguration: "Collection Hook", hookConfigurationNote: "Only one Forge collection Hook is active at a time. Switching hosts removes the previous host's Forge Hook while preserving other user Hooks; each project still controls collection through its enabled Skills.",
      codexTrustTitle: "Trust the Codex user Hook", codexTrustSteps: "After applying the Codex Hook, open Codex Desktop → Settings → Hooks. Find the user Hook, review its command and select Trust, then start a new Codex session to reload the Hook configuration.",
      ...
      claudeTrustTitle: "Claude Code Hook status", claudeTrustNote: "This page currently verifies only the configuration file and Hook receipts. Claude Code's native trust or approval state is unverified, so it is never reported as trusted.",
      cursorTrustTitle: "Cursor Hook status", cursorTrustNote: "Cursor native Hooks are still being verified. This page reports configuration and observed receipts without presenting unknown states as failures.",
      globalSetting: "Local user setting",
      codexHost: "Codex", claudeCodeHost: "Claude Code", cursorHost: "Cursor",
      selectedGlobally: "Selected on this machine", globalHookReady: "Local user Hook installed",
      ...
      applyHook: "Apply selection", removeHook: "Remove local Hook",
      hookConfigured: "Switched the local collection host to {host}", hookRemoved: "Removed the local collection Hook",
      ...
      removeHookConfirm: "Remove the local user collection Hook? Skill-contract capture will remain available.",
      switchHookConfirm: "Switching to {next} removes the Forge Hook from {previous} while preserving other user Hooks. Continue?",
      hookSwitched: "Switched from {previous} to {next}; the previous Forge Hook was removed",
